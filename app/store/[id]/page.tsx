...
function addToCart(product: { id: string; name: string; brand: string; price: number }) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12">
      ...
      <div className="grid md:grid-cols-2 gap-10">
        {/* Image */}
        <div
          className={`bg-gradient-to-br ${product.color} rounded-2xl aspect-square flex items-center justify-center text-9xl`}
        >
          {product.image}
        </div>
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-[#8fa898] text-sm">
            ...
            <ul className="flex flex-col gap-2">
              {product.features.map((f, i) => (
                <li key={i} className="flex items-start gap-2 text-sm text-[#c8c4bc]">
                  <Check size={14} className="text-[#5c9e6e] flex-shrink-0 mt-0.5" />
                  {f}
                </li>
              ))}
            </ul>
          ...
          <button
            ...
            className={`w-full flex items-center justify-center gap-2 py-3.5 rounded-xl font-bold text-base transition-all ${
              added
                ? 'bg-[#2d4a30] text-[#5c9e6e]'
                : 'bg-[#5c9e6e] hover:bg-[#7dba8e] text-[#0d1a0e]'
            }`}
          >
            {added ? (
              <>
          ...
          <div className="grid sm:grid-cols-3 gap-5">
            {related.map((p) => (
              <Link
                key={p.id}
                href={`/store/${p.id}`}
                className="group bg-[#162418] border border-[#2d4a30] rounded-xl overflow-hidden hover:border-[#5c9e6e]/40 hover:-translate-y-1 transition-all"
              >
                <div className={`bg-gradient-to-br ${p.color} h-32 flex items-center justify-center text-5xl`}>
                  {p.image}
                </div>
                <div className="p-4">
                  <p className="text-[#8fa898] text-xs">{p.brand}</p>
                  <p className="text-[#e8e4dc] font-semibold text-sm group-hover:text-[#7dba8e] transition-colors">
                    {p.name}
                  </p>
